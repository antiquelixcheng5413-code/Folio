...
function SkillsView({ language, knowledge, onOpen }: { language: Language; knowledge: Record<string, unknown>[]; onOpen: () => void }) {
  ...
  return (
    <section className="page page-view">
      <div className="page-title"><div><span className="eyebrow">LEARNING MAP</span><h1>{copy[language].skills}</h1><p>{language === "zh" ? "只保留可学习、可练习、可验证掌握程度的专业技能点；人物和视频结论不会冒充技能。" : "Only learnable, practiceable and assessable professional skills become nodes."}</p></div><div className="saved-pill"><span>{language === "zh" ? "专业技能点" : "SKILL POINTS"}</span><strong>{knowledge.length}</strong></div></div>
      <div className="skill-visual-grid">
        ...
        <article className="knowledge-tree-card"><div className="section-head"><div><span className="eyebrow">SKILL RELATION TREE</span><h2>{language === "zh" ? "专业技能关系树" : "Professional skill tree"}</h2><p>{language === "zh" ? "领域 → 可验证技能点，并显示技能类型与掌握度" : "Domain → assessable skill, with type and mastery"}</p></div></div><div className="knowledge-tree"><div className="tree-root"><PeekMark compact /><strong>{language === "zh" ? "我的能力画像" : "My capability profile"}</strong></div><div className="tree-branches">{treeNodes.map((item, index) => <div className={`tree-node tone-${index % 3}`} key={String(item.id || index)}><i /><span><b>{String(item.domain || (language === "zh" ? "未分类" : "Unclassified"))}</b>{String(item.topic)}</span><small>{String(item.skillType || "concept")} · {masteryOf(item)}%</small></div>)}</div></div></article>
      </div>
      <div className="skill-layout">
        <article className="skill-map-panel"><div className="section-head"><div><h2>{language === "zh" ? "从内容中形成的专业技能点" : "Professional skills from your content"}</h2><p>{language === "zh" ? "每个节点包含能力边界、证据、覆盖度和掌握度估计" : "Each node has scope, evidence, coverage and estimated mastery"}</p></div><div className="map-legend"><span><i className="done" />{language === "zh" ? "掌握度 ≥70" : "Mastery ≥70"}</span><span><i className="doing" />{language === "zh" ? "学习中" : "Developing"}</span></div></div>
      ...
      <div className="detail-body">
        ...
        <aside className="detail-aside">
          ...
          <article className="score-card">
            ...
            {result.personalization && <div className="match-formula"><strong>{language === "zh" ? "Peek Match v2 · 随当前书架动态重算" : "Peek Match v2 · recalculated from your current shelf"}</strong><p>{language === "zh" ? "计算目标内容时会排除它自己，避免自我匹配。" : "The target item is excluded to avoid matching against itself."}</p><div>{[[language === "zh" ? "技能关联" : "Skill relevance", result.personalization.skillFit], [language === "zh" ? "知识增益" : "Knowledge gain", result.personalization.knowledgeGain], [language === "zh" ? "难度适配" : "Difficulty fit", result.personalization.difficultyFit]].map(([label, value]) => <span key={String(label)}><small>{label}</small><b>{value}%</b></span>)}</div><code>({result.personalization.skillFit} × {result.personalization.valueMultiplier.toFixed(2)}) ≈ {result.signals.match}</code></div>}
