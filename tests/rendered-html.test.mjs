...
assert.match(client, /titleMode/);
  ...
  assert.match(client, /searchResults/);
  assert.match(client, /搜索学习库/);
  assert.match(client, /输入标题、来源、知识点或笔记关键词/);
  assert.match(client, /accountDisplayName/);
  assert.match(client, /好名字\.\*朋友\.\*记住/);
  assert.match(client, /useState<string \| null>\(null\)/);
  assert.match(client, /notebook-content-type/);
  assert.match(client, /显示与字号/);
  assert.match(styles, /\.search-popover/);
  assert.match(styles, /\.settings-font-options/);
